% Experiments for the nonlinear discrete Correntropy filter
...
results_filepath = 'results_audio.txt';
fprintf('CORRENTROPY FILTER RESULTS\n');
fd = fopen(results_filepath, 'w');
fprintf(fd, 'CORRENTROPY FILTER RESULTS\n');
fclose(fd);
...
for i = 4:4
    ...
    % Estimate the signal
    d_hat = correntropyFilterCola(x, y, L, sigma, M);
