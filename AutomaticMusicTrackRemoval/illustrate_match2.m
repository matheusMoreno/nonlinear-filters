function illustrate_match2(DQ,SR,DS,SS)
...
tbase = 0.021;  % time base of analysis
%matchtrk = R(1,1);
matchdt = R(1,3);
%[d,sr] = mp3read(FL{matchtrk});
Ld = find_landmarks(DS,SS);
% Plot them, aligning time to the query
subplot(212)
show_landmarks(DS,SS,Ld,matchdt*tbase + [0 length(DQ)/SR]);
%[p,name,e] = fileparts(FL{matchtrk});
%name(find(name == '_')) = ' ';
title(['Match: at ',num2str(matchdt*tbase),' sec']);

% Highlight the matching landmarks
show_landmarks([],SS,Lm,[],'o-g');
subplot(211)
show_landmarks([],SS,Lm-repmat([matchdt 0 0 0],size(Lm,1),1),[],'o-g');
title('Query audio')
end