...
%
% Adapted by: Carlos Lordelo    % changed the initial parameters and Fs, nfft etc
...
targetdf = 63;  % +/- 50 bins in freq (LIMITED TO -64..63 IN LANDMARK2HASH)

%     .. and a time duration (maximum look ahead)
targetdt = 15;  % (LIMITED TO <32 IN LANDMARK2HASH)
...
% We use a 64 ms window (512 point FFT) for good spectral resolution 32 hop (original)
% We use a 42 ms window (2048 point FFT) for good spectral resolution 21 hop (mod)
fft_ms = 42;
fft_hop = 21;
nfft = 2^nextpow2(round(targetSR/1000*fft_ms));
overlap = nfft - 2^nextpow2(round(targetSR/1000*fft_hop));

S = abs(spectrogram(D,nfft,overlap,nfft,targetSR));
% convert to log domain, and emphasize onsets
Smax = max(S(:));
% Work on the log-magnitude surface
S = log10(max(Smax/1e6,S));    
...
%S = S + 20; %ADAPTED BY CARLOS TO FIND PEAKS UNDER -20 DB
...
for i = 1:size(S,2)-1
  ...
  % doesn't fit in 10 bits)
  sdiff(end) = 0;  % i.e. bin 1025 from the sgram
