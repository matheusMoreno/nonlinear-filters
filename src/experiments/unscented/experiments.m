% Experiments for the unscented Kalman filter
...
fprintf('UNSCENTED KALMAN FILTER RESULTS\n');
fd = fopen(results_filepath, 'w');
fprintf(fd, 'UNSCENTED KALMAN FILTER RESULTS\n');
fclose(fd);
...
covs = [1e-4; 4e4; 1e-4; 1e4; 1e-4; 2e3];

parameters_str = strjoin({
    sprintf('\nHistory length: L = %d\n', L),                   ...
    sprintf('Max polynomial size: P = %d\n', P),                ...
}, '');
fprintf(parameters_str);
fd = fopen(results_filepath, 'a');
...
for i = 1:6
    ...
    results_str = strjoin({
        ...
        sprintf('Process variance: %.5f; Observation variance: %.5f\n\n', 1e-2, covs(i)), ...
