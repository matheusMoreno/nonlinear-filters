classdef Rnonlin_calcApp < handle
%
% Usage:
%     app.Rnonlin_calcApp

  properties
      AppPath = {'/Rnonlin_calc/code'};
	  AppClass = 'Rnonlin_calcApp';
	  AppHandle;
	  AppCount = 0;
      Increment = 1;
	  Decrement = 0;
  ...
  methods
    ...
    function startApp(obj)
        ...
        if nargout(@Rnonlintest) == 0  
            eval('Rnonlintest');
        else
			obj.AppHandle = eval('Rnonlintest');   
        end 
        % Increment the reference count by one and lock the file
        mlock;		
        Rnonlin_calcApp.refcount(obj.Increment);
